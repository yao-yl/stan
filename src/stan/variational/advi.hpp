  ...
  namespace variational {
    ...
    public:
      ...
      int run(double eta, bool adapt_engaged, int adapt_iterations,
        ...
        if (msg.str().length() > 0)
          logger.info(msg);
        values.insert(values.begin(), 0);  //  log_p
        values.insert(values.begin(), 0);  //  log_q
        ...
        //  log_p: Log probability in the unconstrained space, for which I drop the Jacobian. log_p is not useful for advi itself, but it is cheap to compute and can guide the diagnostics.
        // TODO:  Ideally I should also remove the constant to reduce computation costs, i.e. log_prob <true, false>, but then it magically becomes 0. --Yuling
          log_p = model_.template log_prob<false, false>(cont_params_, &msg2);
          if (msg2.str().length() > 0)
            logger.info(msg2);
          values.insert(values.begin(), log_p);
          values.insert(values.begin(), log_q);
          parameter_writer(values);
        }
